...
<div class="card">
    <div class="card-header">
        Approved {{ trans('global.agent.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.agents.unblockprocess") }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')   
                     
            <div class="form-group {{ $errors->has('status_block') ? 'has-error' : '' }}">
            <div class="checkbox">
            <label>Buka {{ trans('global.agent.fields.status_block') }}*</label>
            <input type="checkbox" data-toggle="toggle" name="status_block" id="status_block">    
                ...
                <p class="helper-block">
                    {{ trans('global.agent.fields.status_block_helper') }}
                </p>
