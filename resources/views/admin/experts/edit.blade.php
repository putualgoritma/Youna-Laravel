...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.expert.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form action="{{ route("admin.experts.update", [$expert->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('register') ? 'has-error' : '' }}">
                <label for="register">{{ trans('global.expert.fields.register') }}*</label>
                <input type="date" id="register" name="register" class="form-control" value="{{ old('register', isset($expert) ? $expert->register : '') }}" required>
                ...
                <p class="helper-block">
                    {{ trans('global.expert.fields.register_helper') }}
                </p>
            </div>
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                <label for="code">{{ trans('global.expert.fields.code') }}*</label>
                <input type="text" id="code" name="code" class="form-control" value="{{ old('code', isset($expert) ? $expert->code : '') }}" required>
                @if($errors->has('code'))
                    <em class="invalid-feedback">
                        {{ $errors->first('code') }}
                    </em>
            ...
            <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                ...
                <select name="type" class="form-control">
                    <option value="expert"{{(old('type', $expert->type) == 'expert' ? 'selected' : '')}}>Expert</option>                    
                </select>
                @if($errors->has('type'))
                    <em class="invalid-feedback">
                        {{ $errors->first('type') }}
