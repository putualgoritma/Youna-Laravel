...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.clinic.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form action="{{ route("admin.clinics.update", [$clinic->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                ...
                <input type="text" id="code" name="code" class="form-control" value="{{ old('code', isset($clinic) ? $clinic->code : '') }}">
                @if($errors->has('code'))
            ...
            <div class="form-group {{ $errors->has('img') ? 'has-error' : '' }}">
                <label for="img">{{ trans('global.clinic_image.fields.name') }}*</label>
                <input type="file" id="img" name="img" class="form-control" value="{{ old('img', count($clinic->images)>0 ? $clinic->images[0]->name : '') }}">
                @if($errors->has('img'))
                    <em class="invalid-feedback">
                        {{ $errors->first('img') }}
                    </em>
                @endif
                <p class="helper-block">
                    {{ trans('global.clinic_image.fields.name_helper') }}
                </p>
                <p>
                    <img src="{{ old('img', count($clinic->images)>0 ?  $clinic->images[0]->path : '') }}" alt="{{ old('name', count($clinic->images)>0 ?  $clinic->images[0]->name : '') }}" width="300">
                </p>
            </div>
