...
trait TraitModel
{
    ...
    {
        if ($type == "order-reservation") {
            $account = Order::where('type', 'reservation')
                ->orderBy('id', 'desc')
                ->first();
            if ($account && (strlen($account->code) == 8)) {
                $code = $account->code;
            } else {
                $code = acc_codedef_generate('RSV', 8);
            }
        }
        
        if ($type == "clinic_customer") {
            $account = ClinicCustomer::orderBy('id', 'desc')
                ->first();
            if ($account && (strlen($account->code) == 8)) {
                $code = $account->code;
            } else {
                $code = acc_codedef_generate('CLC', 8);
            }
        }
        if ($type == "clinic") {
            $account = Clinic::orderBy('id', 'desc')
                ->first();
            if ($account && (strlen($account->code) == 8)) {
                $code = $account->code;
            } else {
                $code = acc_codedef_generate('CLN', 8);
            }
        }
        if ($type == "expert") {
            $account = Customer::where('type', 'expert')
                ->orderBy('id', 'desc')
                ->first();
            if ($account && (strlen($account->code) == 8)) {
                $code = $account->code;
            } else {
                $code = acc_codedef_generate('EXP', 8);
            }
        }
